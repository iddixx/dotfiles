...
def underlined_tags(self, painter): # self is a HLWMTagInfo object
    ...
    painter.fg(MY_FG if self.occupied else MY_FG) # i changed colors here
    if self.urgent:
        ...
        painter.fg(FG)  # i changed colors here
...
class HLWMTagInfo:
    def __init__(self):
        ...
        self.visible = True
        self.empty = False
        self.activecolor = MY_ACTIVE_FG
        self.emphbg = MY_BG_BRIGHT
        self.index = 0
    ...
    def render(self, painter):
        ...
        painter.fg(None if self.occupied else FG)
        if self.urgent:
            painter.fg(FG) # i changed colors here 
            painter.bg(BG) # i changed colors here
            painter.set_flag(Painter.overline, False)
        if self.focused:
            painter.fg(FG) # i changed colors here
            painter.ol(self.activecolor)
        else:
            painter.ol(BG) # i changed colors here
        painter.space(4)
        painter += self.name
        painter.space(4)
        painter.bg()
